...
#if GASNET_PSHM
// Cross-map one remote shared segments
// TODO/WIP: Idempotence to avoid duplicate mapping and tracing
static
                ...
                int is_aux)
{
    ...
    uintptr_t offset = (uintptr_t)segbase - (uintptr_t)addr;
    if (is_aux) {
        gasneti_nodeinfo[jobrank].auxoffset = offset;
    } else {
        gasneti_nodeinfo[jobrank].offset = offset;
    }

    GASNETI_TRACE_PRINTF(C, ("Remote segment %d: segbase="GASNETI_LADDRFMT"  segsize=%"PRIuPTR,
                             (int)jobrank, GASNETI_LADDRSTR(segbase), size));
}
...
void gasneti_segment_cross_map(const gasnet_seginfo_t *seginfo, int is_aux)
{
    gasneti_nodeinfo[gasneti_mynode].offset = 0;
    gasneti_pshm_rank_t pshm_rank = 0;

    gasneti_publish_segment(seginfo[gasneti_mynode]); // WIP - barrier synchronization
                           ...
                           gex_Flags_t flags)
{
  ...
  gasneti_segment_cross_map(all_segments, is_aux);
  gasneti_pshm_cs_leave();
#endif
...
/* ------------------------------------------------------------------------------------ */
extern int gasneti_Segment_CirculateM(
                gex_TM_t               *e_tm,
                size_t                 num_tm,
                gex_Flags_t            flags)
{
  ...
  GASNETI_TRACE_PRINTF(C,("gex_Segment_CirculateM: num_tm=%"PRIuSZ", flags=%d", num_tm, flags));
  for (gex_Rank_t i = 0; i < num_tm; ++i) {
    GASNETI_TRACE_PRINTF(C,("gex_Segment_CirculateM:    tm[%u] = "GASNETI_TMSELFFMT,
                            i, GASNETI_TMSELFSTR(e_tm[i])));
  }

  if (! e_tm) {
    gasneti_fatalerror("Invalid call to gex_Segment_CirculateM() with NULL tm[]");
  }
  if (!num_tm) {
    gasneti_fatalerror("Invalid call to gex_Segment_CirculateM() with zero num_tm");
  }
  if (flags) {
    gasneti_fatalerror("Invalid call to gex_Segment_CirculateM() with non-zero flags");
  }
  if (num_tm > 1) {
  ...
  // Variable HCA count complicates allocation and use
  ...
  for (size_t i = 0; i < team_sz; ++i, ++p) {
    ...
    if (loc.gex_rank == gasneti_mynode) {
      ...
      gasneti_unreachable_error(("gasnetc_Segment_CirculateM does not yet handle non-primordial EPs"));
    }
  }

  // BIG-TODO: cross-mapping ??
