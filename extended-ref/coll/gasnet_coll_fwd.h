...
/* stats needed by the COLL reference implementation */
  ...
  #define GASNETI_COLL_STATS(CNT,VAL,TIME)    \
        ...
        VAL(I, COLL_P2P_ALLOC, bytes)         \
        VAL(I, COLL_P2P_FREE, bytes)          \
        VAL(I, COLL_OP_ALLOC, bytes)          \
        VAL(I, COLL_OP_FREE, bytes)           \
        VAL(I, COLL_GDATA_ALLOC, bytes)       \
        VAL(I, COLL_GDATA_FREE, bytes)        \
        VAL(I, COLL_EOP_ALLOC, bytes)         \
        VAL(I, COLL_EOP_FREE, bytes)          \
        /*END*/
#endif
