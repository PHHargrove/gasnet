...
int gasnetc_ofi_init(void)
{
  ...
  // Address bug 4553 by attempting to remove FI_MR_PROV_KEY if present
  ...
  if (info->domain_attr->mr_mode & FI_MR_PROV_KEY) {
    ...
    if (alt_info && !strcmp(info->fabric_attr->prov_name,
                            alt_info->fabric_attr->prov_name)
                 && !strcmp(info->domain_attr->name,
                            alt_info->domain_attr->name)) {
      fi_freeinfo(info);
      info = alt_info;
    } else {
