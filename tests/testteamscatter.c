/* $Source: bitbucket.org:berkeleylab/gasnet.git/tests/testteamscatter.c $
 * See license.txt for terms
 * Rajesh Nishtala and Yili Zheng
 */

/* Description: basic GASNet team implementation test, team scatter
   test, and team broadcast test.  Column teams and row teams of a process grid are created and
   team barriers and scatters are performed on these teams. */
...
int main(int argc, char **argv) 
{
  ...
  test_init("testteamscatter", 1, "(nrows) (ncols) (iters)");
  ...
  maxsz = MAX_SIZE / nodes;
  
  /*first do team all broadcast*/
  for (sz = 1; sz<=maxsz; sz=sz*2) {
    int root = 0;
    ...
    BARRIER();
    gasnet_coll_scatter(GASNET_TEAM_ALL, C_int, root, B_int, sz*sizeof(int), 
                          GASNET_COLL_IN_ALLSYNC|GASNET_COLL_OUT_NOSYNC|GASNET_COLL_LOCAL);
    BARRIER();
    for(i=0; i<sz; i++) {
      int expected = mynode*sz+root*sz+42+i;
      if(expected != C_int[i]) {
    ...
    if(mynode == 0){
      printf("%d> %ld byte scatter team all time: %8.3f usec\n",mynode,(long)sz*sizeof(int),
           ((float)total)/(iters));
      fflush(stdout);
    }
  }
  ...
  for (sz = 1; sz<=maxsz; sz=sz*2) {
    ...
    BARRIER();
    gasnet_coll_scatter(my_row_team, C_int, 0, B_int, sz*sizeof(int), 
                          GASNET_COLL_IN_ALLSYNC|GASNET_COLL_OUT_NOSYNC|GASNET_COLL_LOCAL);
    BARRIER();
    for(i=0; i<sz; i++) {
      int expected = my_col*sz+my_row*ncols*sz+42+i;
      if(expected != C_int[i]) {
        fprintf(stderr, "%d> %d %d (expecting %d)\n", mynode, i, C_int[i], expected);
        gasnet_exit(1);
      }
    ...
    if(my_col == 0){
      printf("%d> %ld byte scatter row team %u time: %8.3f usec\n",mynode,(long)sz*sizeof(int),
  ...
  for (sz = 1; sz<=maxsz; sz=sz*2) {
    ...
    BARRIER();
    gasnet_coll_scatter(my_col_team, C_int, 0, B_int, sz*sizeof(int), 
                          GASNET_COLL_IN_ALLSYNC|GASNET_COLL_OUT_NOSYNC|GASNET_COLL_LOCAL);
    BARRIER();
    for(i=0; i<sz; i++) {
      int expected = my_row*sz+my_col*sz+42+i;
      if(expected != C_int[i]) {
        fprintf(stderr, "%d> %d %d (expecting %d)\n", mynode, i, C_int[i], expected);
        gasnet_exit(1);
      }
    ...
    if(my_row == 0){
      printf("%d> %ld byte scatter col team %u time: %8.3f usec\n",mynode,(long)sz*sizeof(int),
