 ...
 * builds both with and without configure-time enable of support for this device
 * class.
 */
...
int main(int argc, char **argv)
{
...
#if GASNET_HAVE_MK_CLASS_CUDA_UVA
  {
    ...
    if (cuDeviceGetCount(&count) || !count) {
      ...
      gex_EP_PublishBoundSegment(myteam, NULL, 0, 0);
      ...
      CUdeviceptr dptr;
      uint8_t *loc_gpu = NULL;
      if (client_segment) {
        check_cudacall( cuMemAlloc(&dptr, TEST_SEGSZ_REQUEST) );
        loc_gpu = (uint8_t *) dptr;
      }

      GASNET_Safe( gex_MK_Create(&kind, myclient, &args, 0) );
      gex_Segment_t d_segment = GEX_SEGMENT_INVALID;
      GASNET_Safe( gex_Segment_Create(&d_segment, myclient, loc_gpu, TEST_SEGSZ_REQUEST, kind, 0));
      loc_gpu = gex_Segment_QueryAddr(d_segment);
