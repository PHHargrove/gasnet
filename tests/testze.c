...
int main(int argc, char **argv)
{
...
#if GASNET_HAVE_MK_CLASS_ZE
  {
    ...
    } myGPUs[1]; // TODO: increase when implementing multi-device tests
    const int maxGPUs = sizeof(myGPUs) / sizeof(myGPUs[0]);

    //ze_result_t result = zeInit(ZE_INIT_FLAG_GPU_ONLY);
    ze_result_t result = zeInit(0);
    if (result) {
      MSG("GEX_MK_CLASS_ZE: skipped - initialization failed (%d)", (int)result);
    } else { // Device enumeration, obtaining context for the first
      ...
      if (!count) {
        MSG("GEX_MK_CLASS_ZE: skipped - could not find a GPU driver");
      }
    }

    ze_device_handle_t device = myGPUs[0].device;
    args.gex_args.gex_class_ze.gex_zeDevice = device;

    ze_context_handle_t context = myGPUs[0].context;
    args.gex_args.gex_class_ze.gex_zeContext = context;
