 ...
 *
 * Description: Test of gex_TM_Pair() for communication initation
 */
...
static void npmed_ping_handler(gex_Token_t token, void *buf, size_t nbytes) {
  ...
  // Not bothering w/ the complexity of NPAM here since token is indep of TM_Pair
...
int main(int argc, char **argv)
{
  ...
  TEST_SRAND(((unsigned int)TIME() ^ (unsigned int)getpid()) & 0xFFFF);
  ...
  const int num_eps = MIN(1, GASNET_MAXEPS);  // TODO: raise "1" for multi-ep testing
...
#if 0 // TODO: allocate/setup non-primordial endpoints
  {
    ...
    const gex_EP_Capabilities_t ep_caps = 0;
    ...
    const gex_EP_Capabilities_t ep_caps = GEX_EP_CAPABILITY_RMA | GEX_EP_CAPABILITY_AM;
    for (gex_EP_Index_t idx = 1; idx < num_eps; ++idx) {
      GASNET_Safe(gex_EP_Create(eps+i, myclient, ep_caps, 0));
      // maybe? gex_Segment_Create(...);  If distinct segment for each EP, test will need additional changes.
      gex_Segment_EP_Bind(mysegment, eps[idx], 0);
    }
    gex_Segment_TM_Publish(myteam, eps+1, num_eps-1, 0);
  }
#endif
  ...
  for (gex_EP_Index_t loc_idx = 0; loc_idx < num_eps; ++loc_idx) {
    ...
    if (TEST_SECTION_BEGIN_ENABLED()) { // RMA Put tests
      ...
      ev[0] =       gex_RMA_PutNB(pair, next, dst++, &next, rank_sz, GEX_EVENT_NOW, 0);
      ev[1] =    gex_RMA_PutNBVal(pair, next, dst++,  next, rank_sz, 0);
      rc =         gex_RMA_PutNBI(pair, next, dst++, &next, rank_sz, GEX_EVENT_NOW, 0);
      assert(!rc);
      rc =      gex_RMA_PutNBIVal(pair, next, dst++,  next, rank_sz, 0);
      assert(!rc);
      rc =    gex_RMA_PutBlocking(pair, next, dst++, &next, rank_sz, 0);
      assert(!rc);
    ...
    if (TEST_SECTION_BEGIN_ENABLED()) { // AM Tests
      ...
      // TODO: What (if anything) is needed to prevent compiler from discarding these dead-writes.
      //       Of course, actually *using* them shoud be sufficient.
