...
// TODO: multi-ep/tm generalizations
static int
gasnetc_segment_exchange(gasnetc_mem_info_t* mem_info, gex_TM_t tm)
{
  gex_Rank_t team_size = tm ? gex_TM_QuerySize(tm) : gasneti_nodes;
  ...
  */
  ...
  for (gex_Rank_t i = 0; i < team_size; i++) {
    ...
    if (tm) {
      ...
      gasneti_assert(! loc.gex_ep_index); // TODO: multi-ep support
...
extern int gasnetc_Segment_PublishM(
                gex_TM_t               *tm,
                size_t                 num_tm,
                gex_Flags_t            flags)
{
  // Conduit-independent parts
  int rc = gasneti_Segment_PublishM(tm, num_tm, flags);
  ...
  gasnetc_segment_exchange(segment ? segment->mem_info : NULL, tm[0]);
#endif
  ...
  gex_Event_Wait(gex_Coll_BarrierNB(tm[0], 0));
