...
extern uint32_t gasneti_gethostid(void) {
    ...
    if_pf (!myid) {
      ...
      // gethostid() is known to be unreliable or unavailable - always hash the hostname
      #undef GASNETI_GETHOSTID_RETRIES
    #else
      // gethostid() is available - hash the hostname only if it looks unreliable after multiple retries
